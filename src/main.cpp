...
int main() {
    ...
    Point3D P1_r = RoboticsMath::getAfterRotation3D(P1_w, translation, theta, axis);
    Point3D P2_r = RoboticsMath::getAfterRotation3D(P2_w, translation, theta, axis);

    // 输出求转换后 P1 和 P2 坐标
    cout << "P1 在新坐标系（机器人坐标系）下的坐标: (" << P1_r.x << ", " << P1_r.y << ", " << P1_r.z << ")" << endl;
    cout << "P2 在新坐标系（机器人坐标系）下的坐标: (" << P2_r.x << ", " << P2_r.y << ", " << P2_r.z << ")" << endl;

    Point3D P1_w2 = RoboticsMath::getBeforeRotation3D(P1_r, translation, theta, axis);
    Point3D P2_w2 = RoboticsMath::getBeforeRotation3D(P2_r, translation, theta, axis);
